...
{   
    ...
    // cout << "If not working, are you sure that you compiled the main_simp.cpp again?"<<endl;
    // if (argc != 2) {
    ...
    if (argc == 1) {
        ...
        cout<< "If not working, did you run ./submit_input_params already so that run_local_debug/parameters.txt is created?"<<endl;
        index = 0;
        file.open("run_local_debug/parameters.txt");
    } 
    else if (argc == 2) {
...
// Define a JSON object
    ...
    input_json_parse_new(input_json_file, jsonData);

    fstream s_file, f_s_file, s_kernel_file, s_screenshot_file,x_file, f_x_file, x_kernel_file, x_screenshot_file,y_file, f_y_file, y_kernel_file, y_screenshot_file, posfile_delta_t,m_file, Order_parameters_file,v_s_file,v_x_file,v_y_file;
    ...
    // kernel_s.open(jsonData["output_folder"].get<string>() + "/" + jsonData["s_kernel_file"].get<string>(), ios::out);
    // screenshot_s.open(jsonData["output_folder"].get<string>() + "/" + jsonData["s_screen_file"].get<string>(), ios::out);
    ...
    // kernel_x.open(jsonData["output_folder"].get<string>() + "/" + jsonData["x_kernel_file"].get<string>(), ios::out);
    // screenshot_x.open(jsonData["output_folder"].get<string>() + "/" + jsonData["x_screen_file"].get<string>(), ios::out);
    ...
    // kernel_y.open(jsonData["output_folder"].get<string>() + "/" + jsonData["y_kernel_file"].get<string>(), ios::out);
    // screenshot_y.open(jsonData["output_folder"].get<string>() + "/" + jsonData["y_screen_file"].get<string>(), ios::out);

    posfile_delta_t.open(jsonData["output_folder"].get<string>() + "/" + jsonData["delta_t_file"].get<string>(), ios::out);
    // m_file.open(jsonData["output_folder"].get<string>() + "/" + jsonData["m_file"].get<string>(), ios::out);
    ...
    // cout<<"delay_steps="<<delay_steps<<endl;
    ...
    // config_init(s_init, x_init,  y_init,  s_present,  x_present,  y_present,L_box_x,L_box_y, load_file_name, aligned_init, N,gen, dis,jsonData);
    config_init(
    ...
    // boundary_update_XY(x_present,y_present,L_box_x,L_box_y, N, boundary_shift, Delta_L, dis, gen);// This is needed, since if we load particles from a larger box,they might not fit in the smaller one. This still needs change since the loaded box can be twice as large.
    
    // For "No Loading", the boundary_update_XY has been included in config_init; however, for loading files, this is a bit trickier because for larger systems we have to keep track of every particle's shift back to the box, otherwise there will be jumps if the v_0*delta_t is too large.

    // Initializing used time
    start=clock();    start2 = chrono::high_resolution_clock::now();


    // frame_rotate_update(s_present,x_present,y_present, L_box_x, L_box_y, N, frame_rotate, rot_angle);
    ...
    for (int k = 0; k < jsonData["Obs_time_steps"].get<int>(); k++) {
        ...
        // Outputting posfile_s,posfile_x,posfile_y,posfile_m
        // output_pos(
        //     jsonData["write_file"].get<int>(),
        //     k,
        //     jsonData["interval"].get<int>(),
        //     jsonData["write_file_loop_cutoff"].get<int>(),
        //     jsonData["N"].get<int>(),
        //     s_file, x_file, y_file, m_file,
        //     s_recorded[delay_steps], x_recorded[delay_steps], y_recorded[delay_steps]
        // );
        ...
        output_screenshot(
            ...
            s_recorded[delay_steps],
            jsonData["N"].get<int>());
    // cout<<"x[i]="<<x_recorded[delay_steps][0]<<endl; // control for parallel omp; if this value is consistent over simulations then there should be no race condition

    }
    cout<<"End of simulation, x[i]="<<x_recorded[delay_steps][0]<<endl; // control for parallel omp; if this value is consistent over simulations then there should be no race condition
    s_file.close();
    f_s_file.close();
    v_s_file.close();
    // s_kernel_file.close();
